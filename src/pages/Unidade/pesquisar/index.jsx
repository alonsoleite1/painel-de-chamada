import { useForm } from "react-hook-form";
import styles from './styles.module.scss';

const BuscarUnidade = ({  onSearch }) => {
    const { register, handleSubmit, formState: { errors } } = useForm();

    const submitForm = (data) => {
        onSearch(data.nome); // Buscar usuário pelo CPF
    };

    return (
        <div className={styles.container}>
            {/* Formulário de busca */}
            <form onSubmit={handleSubmit(submitForm)} className={styles.formBusca}>
                <label className={styles.label}>Nome:</label>
                <input
                    {...register('nome', { required: 'Este campo é obrigatório' })}
                    className={styles.inputBusca}
                    type="text"
                />
                {errors.nome && <span className={styles.errorBusca}>{errors.nome.message}</span>}

                <button type="submit" className={styles.buttonSearch}>Buscar</button>
            </form>

        </div>
    );
};

export default BuscarUnidade;

